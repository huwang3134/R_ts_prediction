...
stat_file_dir_prefix <- sprintf("out/stat/%s", yesterday_str)
dir.create(stat_file_dir_prefix, showWarnings = FALSE)
xreg_stat_file_dir_prefix <- sprintf("out/stat/xreg/%s", yesterday_str)
dir.create(xreg_stat_file_dir_prefix, showWarnings = FALSE)
error_file_dir_prefix <- sprintf("out/error/%s", yesterday_str)
dir.create(error_file_dir_prefix, showWarnings = FALSE)
png_file_dir_prefix <- sprintf("out/png/%s", yesterday_str)
...
max_p <-
  7 # experimental, significantly slower than default value, especially for seasonal model
...
for (j in seq_along(bid_list_str_list)) {
  ...
  mysql.drv <-
    MySQL()  # new connection each time to avoid time-out problem
  mydb = dbConnect(
    mysql.drv,
    user = 'bigdata_r',
    password = 'B1gD20t0_r',
  ...
  print(sql)
  clip_info_list[[cur_bid_str]] <- dbGetQuery(mydb, sql)
  ...
  # release_date <- as.Date(res_df$releaseTime)  # this is NOT accurate within DB table
  ...
  pid_startTime_df <- dbGetQuery(mydb, clip_startTime_sql)
  ...
  start_date <-
    as.Date(sapply(start_date, function(cur.date)
      ifelse(cur.date < last_feasible_day,
             cur.date,
             last_feasible_day)))
  ...
  for (i in seq_along(pid_str_list)) {
    ...
    xreg_stat_file_path <-
      sprintf("%s/%s_xreg_rela_impo",
              xreg_stat_file_dir_prefix,
              out_file_desc_prefix)
    xreg_lm_png_file_path_prefix <-
      sprintf("%s/%s_xreg_lm_plot",
              xreg_stat_file_dir_prefix,
              out_file_desc_prefix)
    fourier_xreg_lm_png_file_path_prefix <-
      sprintf(
        "%s/%s_fourier_xreg_lm_plot",
        xreg_stat_file_dir_prefix,
        out_file_desc_prefix
      )
    bc_xreg_stat_file_path <-
      sprintf(
        "%s/%s_boxcox_xreg_rela_impo",
        xreg_stat_file_dir_prefix,
        out_file_desc_prefix
      )
    bc_xreg_lm_png_file_path_prefix <-
      sprintf("%s/%s_boxcox_xreg_lm_plot",
              xreg_stat_file_dir_prefix,
              out_file_desc_prefix)
    bc_fourier_xreg_lm_png_file_path_prefix <-
      sprintf(
        "%s/%s_boxcox_fourier_xreg_lm_plot",
        xreg_stat_file_dir_prefix,
        out_file_desc_prefix
      )
    ...
    cur_daily_data$weekday_levels <-
      as.factor(cur_daily_data$weekday_str)
    cur_daily_data$day <-
    ...
    if (using_xreg) {
      ...
      tryCatch({
        cur_bc_tsoutliers <-
          tso(bc_train_clean_cnt_sample_ts,
              types = c("AO", "LS", "TC")) # Error in rowSums(xregg) : 'x' must be an array of at least two dimensions
        ...
        error(logger,
              sprintf("BoxCox tsoutliers::tso:\t'%s'", as.character(e)))
        sink(cur_err_out)
        print(as.character(e))
        traceback()
        sink()
      ...
      if (!tsoutliers_na && tsoutliers_exist) {
        ...
        cur_xreg_fourier_var_cnt <-
          ifelse(is.null(cur_xreg_fourier), 0, ncol(cur_xreg_fourier))
      ...
      if (!bc_tsoutliers_na && bc_tsoutliers_exist) {
        ...
        cur_bc_xreg_var_cnt <-
          ifelse(is.null(bc_xreg), 0, ncol(bc_xreg))
        ...
        cur_bc_xreg_fourier_var_cnt <-
          ifelse(is.null(cur_bc_xreg_fourier),
                 0,
                 ncol(cur_bc_xreg_fourier))
      ...
      if (train_size > 7 + 7 * 2) {
        ...
        best_fit <- Inf
        cur_trial_ts <-
          ts(bc_train_weekly_cnt_ts, frequency = 7 * 2)
      ...
      if (any(cur_daily_data$is_holiday)) {
        ...
        cur_covariates <-
          cbind(
            model.matrix(~ cur_daily_data$weekday_levels)[,-1],
            # remove reference level/intercept
            cur_daily_data$is_holiday,
            cur_daily_data$day
          )
      ...
      if (cur_xreg_valid) {
        ...
        if (is.rank.deficient(xreg)) {
          ...
          detach(d_cor_melt)
          if (nrow(cor_var_df)) {
            error(logger, format(head(cor_var_df))) # output format is weird
          }
      ...
      if (cur_bc_xreg_valid) {
        ...
        if (is.rank.deficient(bc_xreg)) {
          ...
          detach(d_cor_melt)
          if (nrow(cor_var_df)) {
            error(logger, format(head(cor_var_df)))
          }
      ...
      if (cur_xreg_fourier_valid) {
        ...
        if (cur_xreg_fourier_valid &&
          ...
          if (nrow(fourier_cor_var_df)) {
            error(logger, as.character(head(fourier_cor_var_df)))
          }
      ...
      if (cur_bc_xreg_fourier_valid) {
        ...
        if (length(bc_fourier_uniq_colnames)) {
          # remove duplicated columns
          cur_bc_xreg_fourier <-
            cur_bc_xreg_fourier[, bc_fourier_uniq_colnames]
          cur_bc_fxreg_fourier <-
        ...
        if (is.rank.deficient(cur_bc_xreg_fourier)) {
          ...
          bc_fourier_d_cor_melt <-
            arrange(melt(bc_fourier_d_cor),-abs(value))
          attach(bc_fourier_d_cor_melt)
          bc_fourier_cor_var_df <-
            bc_fourier_d_cor_melt[which(Var1 != Var2 &
                                          value > 0.5), ]
          detach(bc_fourier_d_cor_melt)
          if (nrow(bc_fourier_cor_var_df)) {
            error(logger,
                  sprintf(
                    "BoxCox transformed fourier terms: %s",
                    format(head(bc_fourier_cor_var_df))
                  ))
          }
          require("usdm")
      ...
      if (cur_xreg_valid) {
        ...
        tryCatch({
          cur_fit <-
            auto.arima(
              # slow as hell due to many outliers when multiple years VV present compared with BoxCox transformed smooth version
              cur_train_sample,
              max.p = max_p,
              max.q = max_q,
              max.d = max_d,
        ...
        if (!cur_fit_na) {
          ...
          stopifnot(length(forecast_list) == length(forecast_title_list))
      ...
      if (cur_xreg_fourier_valid) {
        ...
        tryCatch({
          # time consuming when multiple years daily VV is present
          cur_fit <- auto.arima(
            cur_train_sample,
            max.p = max_p,
            max.q = max_q,
        ...
        if (!cur_fit_na) {
          ...
          cur_model_name <- as.character(cur_fit)
          cur_model_name <-
            sprintf("%s using fourier terms etc.", cur_model_name)
          cur_forecast_title <- sprintf(
            "%s '%s' %d points, Forecasts from %s\nMean Absolute Percentage Error = %.4f, first day Percentage Error = %.4f",
            cur_bid_name,
            cur_pid_name,
      ...
      if (cur_bc_xreg_fourier_valid) {
        ...
        if (!cur_fit_na) {
          ...
          cur_model_name <-
            sprintf(
              "BoxCox-transformed(lambda=%.2f) %s using fourier terms etc.",
              lambda,
              cur_model_name
            )
          cur_forecast_title <- sprintf(
          ...
          cur_xreg_colnames_str <-
            paste(cur_xreg_colnames, collapse = ",")
          info(
            logger,
            sprintf(
              "%s '%s' %s 耗时：%s, xreg_fitted_mape=%.4f, mape=%.4f, 首日预测误差%.4f\n\t%d predictors: %s",
    ...
    cur_fit_na <- FALSE
    train_x_new <-
      msts(train_sample_ts, seasonal.periods = c(7, 7 * 2))
    test_x_new <-
      msts(test_vv_count_ts, seasonal.periods = c(7, 7 * 2))
    actual_x_new <-
      msts(actual_vv_count_ts, seasonal.periods = c(7, 7 * 2))
    ...
    fit.start.time <- Sys.time()
    cur_fit <-
      tbats(cur_train_sample) # https://robjhyndman.com/hyndsight/forecasting-weekly-data/
    # https://robjhyndman.com/publications/complex-seasonality/
    fit.end.time <- Sys.time()
    fit.time.taken <- fit.end.time - fit.start.time
    cur_fcast <-
      forecast(cur_fit, h = predict_ts_len, level = c(0.8, 0.95, 0.99))
    cur_fcast_value <- cur_fcast$mean
    cur_first_day_ape <-
      mape(as.numeric(cur_test_sample[1]),
           as.numeric(cur_fcast_value[1]))
    ...
    fit.start.time <- Sys.time()
    cur_fit <-
      ets(cur_train_sample) # https://robjhyndman.com/hyndsight/forecasting-weekly-data/
    # https://robjhyndman.com/publications/complex-seasonality/
    fit.end.time <- Sys.time()
    fit.time.taken <- fit.end.time - fit.start.time
    cur_fcast <-
    ...
    fit.start.time <- Sys.time()
    cur_fit <-
      baggedETS(cur_train_sample) # https://robjhyndman.com/hyndsight/forecasting-weekly-data/
    # https://robjhyndman.com/publications/complex-seasonality/
    fit.end.time <- Sys.time()
    fit.time.taken <- fit.end.time - fit.start.time
    cur_fcast <-
    ...
    tryCatch({
      cur_fit <- auto.arima(
        ...
        stepwise = FALSE,
        seasonal = TRUE,
        # not effective if frequency is 1
        trace = FALSE
        # method = "ML"
        # trace = FALSE,
        # # trace = TRUE
    ...
    choosen_xreg_fitted_mape <-
      xreg_fitted_mape_list[[best_model_idx]]
    ...
    cur_test_mean_abs_vibr_ratio <-
      mean(abs(
        tail(cur_daily_data$clean_cnt_change_ratio, predict_ts_len)
      ))
    cur_first_day_change_ratio <-
      cur_daily_data$clean_cnt_change_ratio[train_size + 1]
    cur_info_pack <-
      c(
        pid = cur_pid,
